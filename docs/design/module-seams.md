...
| 领域事实 | 唯一写所有者 | 必须保持的分离 |
|---|---|---|
| 账号、密码、邮箱状态、验证／恢复令牌、登录会话 | `identity` | 账号不是成员；登录会话不是房间访问会话或认领 |
...
| 房间、主持权、房间访问会话、观众会话、人员牌、访客牌、认领、队伍、容量、对局记录、房间修订号、房间快照、对局快照、房间内头像引用 | `activity` | 房间快照不是对局快照；房间访问会话不是登录会话；认领不是身份或分队 |
