...
| 开放房间恰有一个合格主持人 | FK 保证引用，唯一当前主持关系 | 锁 room、旧/新 member | 启用、绑定、团队一致、接管资格 |
| 同房间一张牌最多一个有效认领；参与会话最多认领一张牌 | 两个 partial unique 约束 | 锁 room、card、session | 会话有效、权限 |
...
| 访客牌最多归属一个成员且目标身份唯一 | FK、访客归属唯一约束 | 锁 room、card、目标 member/profile | 主持权限、同项目、资料规则 |
...
| 同牌认领 | room → card → participant session | 牌与会话都未有有效认领 |
...
- team：成员启停/恢复、绑定/解绑/离开、管理员授予/撤销、邀请签发/撤销/接受、项目创建/停用/恢复、档案创建/修改/停用/恢复、模板发布/字段变更。
